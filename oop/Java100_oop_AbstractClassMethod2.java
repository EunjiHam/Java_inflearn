...
public class Java100_oop_AbstractClassMethod2 {
    public static void main(String[] args){
        ...
        // 따라서, 부모 클래스로서의 역할은 하지만, 구체적인 사용은 자식 클래스에서의 재정의(오버라이딩)하여 사용해야 한다. --> 강제성
        ...
        // 즉, 일반 메서드로 구현하면 누군가는 해당 메서드를 구현하지 않을 수도 있다.
        // 무조건 상속받은 자식 클래스 입장에서는 추상 메서드를 재정의해서 구현하도록 하기 위함이다.
        // 위의 cat 클래스에서 재정의한 cry() 메서드를 지워보자. --> 에러가 나는 이유는 추상 메서드를 재정의(오버라이딩)하지 않아서이다.
        

        // 꼭 재정의(override) 해야만 하는가?
        // 일단 그렇다.
        // 자식 클래스는 일단은 무조건 부모(추상) 클래스로부터 상속받은 추상 메서드는 오버라이딩해서 재정의해야한다.
        ...
        // 이러한 추상 클래스는 여러명의 개발자가 작업할 때 코드의 확장성과 분업을 효율적으로 처리할 수 있게 해준다.
        // 분업하된 시스템에서 공통의 프로젝트를 진행할 때 많이 사용되어지는 중요한 문법이다.
    }
}
