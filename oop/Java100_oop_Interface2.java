...
// 추상 클래스와 같이 메서드의 구체적인 내용은 기술되어있지 않으므로 인터페이스를 상속받은 클래스에서 재정의(오버라이딩)하여 사용.

// [2]: 상속
// 클래스는 "단일 상속"만 가능, 인터페이스는 "다중 상속"이 가능 --> 가장 큰 차이점.
// class --> extends, interface --> impelements --> 다중 상속을 구현 --> A,B --> 콤마(,)로 분리
...
// 상속을 받는 extends 키워드와 구현을 하는 impelements 키워드가 동시에 쓰일 때 --> extends 키워드가 항상 먼저 쓰인다.
